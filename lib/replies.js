...
function ReplyFactory(opts, aum, acm) {
  this.VERSION_STRING = opts.version;
  this.SERVER_STRING = opts.servername;
  this.AVAIL_USER_MODES = aum;
  this.AVAIL_CHAN_MODES = acm;
}
...
ReplyFactory.prototype.welcomeMessage = function (user) {
  ...
  const myinfo = this.createReply('RPL_MYINFO', user)
    ...
    .replace('<available user modes>', this.AVAIL_USER_MODES)
    .replace('<available channel modes>', this.AVAIL_CHAN_MODES);

  const { remoteAddress, remotePort } = user;
