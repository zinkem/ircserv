...
class Server {
  ...
  constructor(opts) {
    ...
    this.inform = new InformLogger(opts.debug);
    this.inform.pipe(fs.createWriteStream(`${opts.logs}/${SERVER_LOG}`));

    // barf -- better change this!! temporary fix
    this.users = require('./usertable.js');
  }
  ...
  listen(port) {
    ...
    const server = net.createServer((socket) => {
      ...
      const command_list = new CommandList({ version: this.version,
