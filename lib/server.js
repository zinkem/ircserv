...
const populateConnectionInfo = (sock, user) => {
  ...
  dns.reverse(lookup_addr, (err, hostnames) => {
    if (err) {
      ...
      inform.log(['Could not find hostname for address', lookup_addr].join(' '));
...
class Server {
  ...
  createConnectionListener() {
    ...
    return (socket) => {
      ...
      clientShell.on('pipe', () => {
        inform.log(`${remoteAddress} connected`);
      });

      clientShell.on('unpipe', () => {
        inform.debug('Socket:', remoteAddress, 'disconnected');
      });
  ...
  getChannel(chan) {
    return this.channels.channels[chan];
  }
