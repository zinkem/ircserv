...
CommandList.prototype.configModeChannel = function (user, chan, flags_arg, params) {
...
CommandList.prototype.OPER = function (user, args) {
  ...
  */
  ...
  return reply.createError(400, user, 'OPER not yet implemented');
};
...
CommandList.prototype.JOIN = function (user, args) {
  ...
  */
  ...
  chan_list.forEach((chan_name) => {
    ...
    // is channel full?
    if (channels[chan]
        && channels[chan].listenerCount('data') + 1 > channels[chan].mode.l
    ...
    channels[chan].write(`:${user.id} JOIN :${chan}\n`);

    user.socket.write(`${user.command_list.TOPIC(user, [chan])}\n`);
    user.command_list.NAMES(user, [chan]);
    user.socket.write(reply.createReply('RPL_CREATIONTIME', user, `${chan} ${channels[chan].created}\n`));
  });
...
CommandList.prototype.PART = function (user, args) {
  ...
  */
  ...
  const chan = args.shift();
  channels[chan].write(`:${user.id} PART ${chan} :${args.join(' ')}\n`);
...
CommandList.prototype.MODE = function (user, args) {
  ...
  */
  ...
  if (channels[chan]) return this.configModeChannel(user, chan, flags, params);
...
CommandList.prototype.TOPIC = function (user, args) {
  ...
  */
  ...
  if (args[2]) {
    ...
    channels[chan].write(`${reply.createReply('RPL_TOPIC', user, [chan, channels[chan].topic].join(' '))}\n`);
  } else if (args[1]) {
    const new_topic = args.slice(1);
    channels[chan].topic = `:${new_topic}`;
    channels[chan].write(`${reply.createReply('RPL_TOPIC', user, [chan, channels[chan].topic].join(' '))}\n`);
  } else {
    return reply.createReply('RPL_TOPIC', user, [chan, channels[chan].topic].join(' '));
  }
...
CommandList.prototype.LIST = function (user, args) {
  ...
  */
  ...
  const chanlist = args[0] ? args[0].split(',') : Object.keys(channels);

  user.socket.write(`${reply.createReply('RPL_LISTSTART', user)}\n`);

  chanlist.filter(k => channels[k] && !channels[k].mode.p && !channels[k].mode.s)
    .forEach((chan_name) => {
      user.socket.write(`${
        reply.createReply('RPL_LIST', user)
          .replace(/<channel>/g, chan_name)
          .replace(/:<topic>/g, channels[chan_name].topic)
          .replace(/<# visible>/g, channels[chan_name].listenerCount('data') - 1)}\n`);
    });
...
CommandList.prototype.INVITE = function (user, args) {
  ...
  */
  ...
  if (!chan_stream && user.channels[chan]) {
  ...
  users[nick].write(`:${sender} INVITE ${args.join(' ')}`);
...
CommandList.prototype.VERSION = function (user, args) {
  ...
  */
  ...
  return reply.createReply('RPL_VERSION', user, VERSION_STRING);
};
...
CommandList.prototype.ADMIN = function (user, args) {
  /* RFC 1459
  ...
  */
  ...
  return reply.createError(400, user, 'ADMIN not yet implemented');
};

CommandList.prototype.INFO = function (user, args) {
  /* RFC 1459
  ...
  */
  ...
  return reply.createError(400, user, 'INFO not yet implemented');
};
...
CommandList.prototype.WHO = function (user, args) {
  ...
  */
  ...
  if (query_string && channels[chan_id]) {
    channels[chan_id]._readableState.pipes
      .filter(listener => listener.owner)
      .forEach((listener) => {
        const member = listener.owner;

        if (ops_flag && !channels[chan_id].mode.o[member.nick]) return;

        user.socket.write(`${
          reply.createReply('RPL_WHOREPLY', user)
            .replace('<channel>', query_string)
            .replace('<user>', member.username)
            .replace('<host>', member.hostname)
            .replace('<server>', SERVER_STRING)
            .replace('<nick>', member.nick)
            .replace('<H|G>[*][@|+]', 'G')
            .replace('<hopcount>', '*')
            .replace('<real name>', member.real_name)}\n`);
      });
  } else {
    Object.keys(users).forEach((user_nick) => {
      const cuser = users[user_nick];
...
CommandList.prototype.WHOIS = function (user, args) {
  ...
  */
  ...
  return reply.createError(400, user, 'WHOIS not yet implemented');
};
...
CommandList.prototype.KILL = function (user, args) {
  ...
  */
  ...
  return reply.createError(400, user, 'KILL not yet implemented');
};
...
CommandList.prototype.PING = function (user, args) {
  ...
  */
  ...
  if (users[dest]) {
    users[dest].write(`:${user.id} PING ${dest}`);
  }
