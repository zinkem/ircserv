...
function CommandList(server, opts) {
  ...
  this.server = server;
  this.rf = new ReplyFactory(opts, server.usermodes, AVAIL_CHAN_MODES);
  return this;
}
...
CommandList.prototype.USER = function (user, args) {
  ...
  */
  ...
  user.on('pipe', (src) => {
    ...
    inform.debug('User', user.username, 'pipe', src.name);
    user.channels[src.name] = true;
  });

  user.on('unpipe', (src) => {
    inform.debug('User', user.username, 'unpipe', src.name);
    if (user.channels[src.name]) delete user.channels[src.name];
  });

  user.on('finish', () => {
    inform.debug('User', user.username, 'finish');
  ...
  user.on('end', () => {
    inform.debug('User', user.username, 'end');
    if (this.users[user.nick]) {
      delete this.users[user.nick];
    }
  });
...
CommandList.prototype.MODE = function (user, args) {
  ...
  */
  ...
  if (this.channels[target]) {
    ...
    inform.debug(result);
    if (result.error) {
      if (result.error === 'NO_PRIVS') response.push(reply.createError('ERR_CHANOPRIVSNEEDED', user).replace('<channel>', target));
      if (result.error === 'NO_USER') response.push(reply.createError('ERR_NOSUCHNICK', user).replace('<nickname>', params[0]));
    } else if (result.add) {
      this.channels[target].write(`:${user.id} MODE ${target} ${flags} ${params_string}\n`);
