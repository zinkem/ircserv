...
function CommandList(server, opts) {
  ...
  this.server = server;
  reply = new ReplyFactory(opts, server.usermodes, AVAIL_CHAN_MODES);
  return this;
}

CommandList.prototype.parse_command = function (shell, tokens) {
  const user = shell.client;
...
CommandList.prototype.NICK = function (user, args) {
  ...
  */
  ...
  if (!old_nick && user.id) {
    user.write(`${reply.welcomeMessage(user)}\n`);
  }
...
CommandList.prototype.USER = function (user, args) {
  ...
  */
  ...
  user.on('pipe', (src) => {
    ...
    inform.debug('UserPassThrough', user.username, 'pipe', src.name);
    user.channels[src.name] = true;
  });

  user.on('unpipe', (src) => {
    inform.debug('UserPassThrough', user.username, 'unpipe', src.name);
    if (user.channels[src.name]) delete user.channels[src.name];
  });

  user.on('finish', () => {
    inform.debug('UserPassThrough', user.username, 'removing user on drain');
  ...
  user.on('end', () => {
    inform.debug('UserPassThrough', user.username, 'removing user on end');
    if (this.users[user.nick]) {
      delete this.users[user.nick];
    }
  });
...
CommandList.prototype.QUIT = function (user, args) {
  ...
  */
  ...
  Object.keys(mchan).forEach((key) => {
    inform.debug(key);
    if (mchan[key]) {
      this.channels[key].write(`:${user.id} QUIT ${args.join(' ')}\n`);
    }
  });
  user.end();
};
  ...
  */
  ...
  chan_list.forEach((chan_name) => {
    ...
    if (this.channels[chan]
      ...
      user.write(`${reply.createError('ERR_INVITEONLYCHAN', user).replace('<channel>', chan)}\n`);
      return;
    }
    ...
    if (this.channels[chan]
      ...
      if (this.channels[chan].mode.k !== pass) {
        user.write(`${reply.createError('ERR_BADCHANNELKEY', user).replace('<channel>', chan)}\n`);
        return;
      }
    }
    ...
    if (this.channels[chan]
      ...
      user.write(`${reply.createError('ERR_CHANNELISFULL', user)}\n`);
      return;
    }
    ...
    user.write(`${this.TOPIC(user, [chan])}\n`);
    this.NAMES(user, [chan]);
    user.write(reply.createReply('RPL_CREATIONTIME', user, `${chan} ${this.channels[chan].created}\n`));
  });
...
CommandList.prototype.MODE = function (user, args) {
  ...
  */
  ...
  if (this.channels[target]) {
    const result = this.channelTable.configModeChannel(user, target, flags, params, reply);
    inform.debug(result);
    if (result.error) {
      if (result.error === 'NO_PRIVS') return reply.createError('ERR_CHANOPRIVSNEEDED', user).replace('<channel>', target);
    } else if (result.add) {
      this.channels[target].write(`:${user.id} MODE ${target} ${flags} ${params[0] ? params[0] : ''}\n`);
    } else if (result.remove) {
      this.channels[target].write(`:${user.id} MODE ${target} ${flags} ${params[0]}\n`);
    } else if (result.report) {
      return reply.createReply('RPL_CHANNELMODEIS', user, `${target} ${result.report}`);
    }
    return '';
  }
  ...
  if (target !== user.nick) {
    return reply.createError('ERR_USERSDONTMATCH', user);
  }

  const response = [];
  const userResult = this.server.configModeUser(target, flags);
  if (userResult.modes || userResult.modes === '') {
    response.push(reply.createReply('RPL_UMODEIS', user)
      .replace('<user mode string>', `+${userResult.modes}`));
  } else {
    if (userResult.add.length > 0) response.push(`:${user.nick} MODE ${target} :+${userResult.add}`);
    if (userResult.remove.length > 0) response.push(`:${user.nick} MODE ${target} :-${userResult.remove}`);
    if (userResult.unknown.length > 0) response.push(reply.createError('ERR_UMODEUNKNOWNFLAG', user));
  }
...
CommandList.prototype.NAMES = function (user, args) {
  ...
  */
  ...
  chanlist.forEach((chan) => {
    if (!this.channels[chan]) {
      user.write(`${reply.createError('ERR_NOSUCHCHANNEL', user).replace('<channel name>', chan)}\n`);
      return;
    }
    ...
    chan_users.forEach((subscriber) => {
      if (subscriber.nick) result.push(subscriber.nick);
    });

    result = result.map(prefix_mode(chan));
    inform.debug(`${chan} listeners`, result);
    inform.debug('ops', this.channels[chan].mode.o);
    user.write(`${reply.createReply('RPL_NAMREPLY', user, `@ ${chan} :${result.join(' ')}`)}\n`);
    user.write(`${reply.createReply('RPL_ENDOFNAMES', user).replace('<channel>', chan)}\n`);
  });
...
CommandList.prototype.LIST = function (user, args) {
  ...
  */
  ...
  user.write(`${reply.createReply('RPL_LISTSTART', user)}\n`);
...
CommandList.prototype.VERSION = function (user) {
  /* RFC 1459
...
CommandList.prototype.TIME = function (user) {
  /* RFC 1459
...
CommandList.prototype.PRIVMSG = function (user, args) {
  ...
  */
  if (args.length < 1) return reply.createError('ERR_NORECIPIENT', user).replace('<command>', 'PRIVMSG');
  if (args.length < 2 || args[1] === ':') return reply.createError('ERR_NOTEXTTOSEND', user);
...
CommandList.prototype.WHOIS = function (user, args) {
  ...
  */
  if (args.length < 1) return reply.createError('ERR_NONICKNAMEGIVEN', user);
...
CommandList.prototype.KILL = function (user, args) {
  /* RFC 1459
     4.6.1 Kill message           STATUS complete
